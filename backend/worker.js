// Minimal backend RUMAT: login + content get/put (persistent), CORS
// Storage: Cloudflare KV (CONTENT, SESSIONS)
// Auth: session-cookie kun salasana oikein (ei salasanaa koodissa!)
...
export default {
  async fetch(req, env) {
    ...
    if (url.pathname === "/api/content" && req.method === "PUT") {
      ...
      if (!authed) return new Response(JSON.stringify({ ok: false, error: "unauthorized" }), { status: 401, headers: { ...headers, ...JSON_TYPE } });
      const body = await req.text(); // raw JSON string
      // Kevyt validointi että on JSON
      try { JSON.parse(body); } catch { return new Response(JSON.stringify({ ok:false, error:"invalid_json" }), { status:400, headers:{...headers, ...JSON_TYPE} }); }
